...
const MessageContent = memo(({ message, isOnline, onCimiteroSelect, onQuestionSelect }: { 
  ...
  if (message.type === 'query' && message.role === 'user') {
    return (
...
export const ChatMessages = forwardRef<HTMLDivElement, ChatMessagesProps>(({
  ...
  // Renderizziamo il contenuto di default se non ci sono messaggi
  ...
  // Renderizza indicatore di elaborazione in corso
  const renderProcessingIndicator = () => {
    if (!isProcessing || messages.length === 0 || 
        (messages.length > 0 && messages[messages.length - 1].content.includes('Sto elaborando'))) {
      return null;
    }
  ...
  const renderProgressBar = () => {
    if (!isProcessing || processingProgress <= 0) return null;
  ...
  return (
    <ScrollArea 
      ref={scrollAreaRef}
      className="h-[calc(100vh-8.5rem)] bg-transparent"
    >
        ...
        {messages.map((message, index) => (
          <div key={index} data-message-index={index} className="animate-fade-in">
            <MessageContent 
              message={message} 
              isOnline={isOnline} 
              onCimiteroSelect={onCimiteroSelect}
